...
| 模式 | 適用場景 | 配置方式 |
|------|---------|---------|
| **各自登入** | 多人多機協作 | github: 每位成員執行 `gh auth login`；gitlab: 執行 `glab auth login --hostname <host>`。Claude Code 自動使用當前登入帳號 |
| **環境變數** | 單機多 Agent | github: 每個 Agent 設定獨立的 `GH_TOKEN`；gitlab: 設定獨立的 `GITLAB_TOKEN` 或在 `~/.config/glab-cli` 維護多 host token |
| **GitHub App / GitLab Bot** | CI/自動化場景 | github: 建立專用 GitHub App，使用 Installation Token；gitlab: 建立 Group/Project Access Token 或 Service Account |
...
flowchart LR
    ...
    CI -->|通過| MAIN["A-Main 初審<br/>讀取 diff"]
    CI -->|失敗| SUB
...
> CLI 指令依專案 remote 自動分派（github: `gh` / gitlab: `glab`），詳見 `vibe-sdlc/references/git-platform-cli.md`。
...
```mermaid
flowchart LR
    SUB["Sub Agent / A-Main<br/>gh pr create / glab mr create<br/>--reviewer H-Director"] --> DIR["H-Director<br/>收到平台通知<br/>審查 & Merge"]
    DIR -->|要求修改| SUB
```
...
| 場景 | 通知方式 |
|------|---------|
| **標準流程** | A-Main 初審通過後執行 `gh pr edit --add-reviewer <H-Director>`（github）或 `glab mr update <N> --reviewer <H-Director>`（gitlab），平台自動發送通知（Email / Web / Mobile） |
| **Bootstrap 階段** | Sub Agent 建立合併請求時直接指定 reviewer：github `gh pr create --reviewer <H-Director>`；gitlab `glab mr create --reviewer <H-Director>` |
| **Milestone 驗收** | A-Main 在 Issue 中 `@H-Director` 並附上驗收檢查清單（github / gitlab 皆支援） |
