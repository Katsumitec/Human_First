...
> **平台支援**：本文件目前以 GitHub（`gh`）為主軸。GitLab（`glab`）的等效配置請見 `vibe-sdlc/references/git-platform-cli.md` §3-§4，並參考下列對應關係：
> - `gh auth login` ⇄ `glab auth login --hostname <gitlab-host>`
> - `GH_TOKEN` 環境變數 ⇄ `GITLAB_TOKEN`（或 per-host token via `glab config set -h <host> token <PAT>`）
> - GitHub App / Installation Token ⇄ GitLab Group/Project Access Token 或 Service Account
> - `gh api repos/{owner}/{repo}/collaborators/...` ⇄ `glab api projects/:id/members`
> - Branch Protection（`gh api .../branches/main/protection`）⇄ GitLab Protected Branches（`glab api projects/:id/protected_branches`）
>
> 完整 GitLab 版多帳號設定文件由 [Vibe-SDLC #5](https://github.com/robin-li/Vibe-SDLC/issues/5) 追蹤。
