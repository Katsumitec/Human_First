...
- **Bootstrap 合併請求**（CI 建立前的任務）直接由 H-Director 審查合併，不經過 CI 閘門與 A-Main 初審。
- Sub Agent 建立合併請求時直接指定 reviewer（依平台分派，見 `../vibe-sdlc/references/git-platform-cli.md` §2.2）：
  - github: `gh pr create --reviewer <H-Director-username>`
  - gitlab: `glab mr create --reviewer <H-Director-username>`
- **CI 就緒後**（如 T-104 合併後），所有後續合併請求必須通過 CI 檢查，恢復標準雙層審查流程。

#### H-Director 通知機制

| 場景 | github 通知方式 | gitlab 通知方式 |
|------|----------------|----------------|
| **Bootstrap 階段** | `gh pr create --reviewer <H-Director-username>`（自動發送 Review Request） | `glab mr create --reviewer <H-Director-username>`（自動指派 reviewer） |
| **標準流程** | A-Main 初審通過後執行 `gh pr edit --add-reviewer <H-Director-username>` | A-Main 初審通過後執行 `glab mr update <N> --reviewer <H-Director-username>` |
| **Milestone 驗收** | A-Main 在 Issue 中 `@H-Director` 並附上驗收檢查清單 | 同左 |

#### PR/MR 審查流程（標準流程，CI 就緒後適用）

```mermaid
flowchart LR
    SUB["Sub Agent<br/>gh pr create / glab mr create"] --> CI["CI Pipeline<br/>(Actions / GitLab CI)"]
    CI -->|通過| MAIN["A-Main 初審"]
    CI -->|失敗| SUB
    MAIN -->|通過| REQ["A-Main 執行<br/>gh pr edit / glab mr update<br/>--add-reviewer H-Director"]
    REQ --> DIR["H-Director<br/>收到平台通知<br/>終審 & Merge"]
    MAIN -->|駁回| SUB
```

1. **Sub Agent** 完成開發後提交合併請求，目標分支為 `main`（或指定的整合分支）。
2. **A-Main** 進行初審：確認範圍僅限該 Agent 負責的目錄、CI 通過、無型別錯誤。
3. A-Main 初審通過後，依平台指派 reviewer（github: `gh pr edit --add-reviewer ...` / gitlab: `glab mr update <N> --reviewer ...`，見 §2.2）通知 H-Director。
...
──────────────────────────
選擇建立方式：
  A 由 AI 代為建立（GitHub: gh repo create / GitLab: glab repo create）
  B 我要自己手動建立
  C 先跳過，稍後再設定
...
**先詢問目標平台**（github / gitlab），再依選擇分派 CLI。

**選項 A — AI 代為建立**：

1. **先檢查對應 CLI 是否可用**（依使用者選的平台，見 `../vibe-sdlc/references/git-platform-cli.md` §4）：
   - github 模式：`gh auth status` 失敗 → 提示 `gh auth login`，未安裝 → 改選 B
   - gitlab 模式：`glab auth status` 失敗 → 提示 `glab auth login --hostname <host>`，未安裝 → 改選 B
2. **互動確認四個資訊**：
   - Repo 名稱（預設為 `basename $(pwd)`，可覆寫）
   - Owner / Group：
     - github: 帳號 or organization（預設為 `gh api user --jq .login` 取得的當前使用者，可覆寫）
     - gitlab: user namespace or group（預設為 `glab api user --jq .username`，可覆寫）
   - 可見性：`private` / `public` / `internal`（預設 `private`；gitlab 三種皆支援）
...
3. **顯示即將執行的指令並請使用者最終確認**，例如：
   ...
   即將執行（gitlab 模式）：
     glab repo create <group>/<name> --private --add-readme=false
...
5. 執行失敗（名稱衝突、權限不足等）時停下報告錯誤，並提供修正建議（改名稱 / 換 owner/group / 檢查 CLI 權限）

**選項 B — 使用者手動建立**：

輸出完整指令清單供使用者複製（依平台分派）：
...
# 或在 GitHub 網頁建立空 repo 後手動加 remote：
