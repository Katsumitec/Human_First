...
2. 讀取所有 `A-*.md` 狀態檔
3. 若無任何狀態檔（首次使用或單 Agent 專案），先呼叫 `detect_remote`，再從 git 平台 Issues 推斷狀態（見 `../vibe-sdlc/references/git-platform-cli.md` §2.1）：
   ```bash
   detect_remote   # 取得 PLATFORM, REPO_SLUG
   if [ "$PLATFORM" = "github" ]; then
     # 進行中的 Issues
     gh issue list -R "$REPO_SLUG" --state open --label "in-progress" --json number,title,assignees
     # 最近關閉的 Issues
     gh issue list -R "$REPO_SLUG" --state closed --limit 5 --json number,title,closedAt
     # Open 合併請求
     gh pr list -R "$REPO_SLUG" --state open --json number,title,headRefName
   else
     # 進行中的 Issues
     glab issue list -R "$REPO_SLUG" --opened --label "in-progress" --output json
     # 最近關閉的 Issues
     glab issue list -R "$REPO_SLUG" --closed --output json | jq '.[:5]'
     # Open 合併請求
     glab mr list -R "$REPO_SLUG" --opened --output json
   fi
   ```

### 步驟 2：收集里程碑數據

依平台分派（見 `../vibe-sdlc/references/git-platform-cli.md` §2.1）：

```bash
if [ "$PLATFORM" = "github" ]; then
  gh issue list -R "$REPO_SLUG" --state all --json number,state,milestone --limit 200
else
  glab issue list -R "$REPO_SLUG" --all --output json
fi
```
