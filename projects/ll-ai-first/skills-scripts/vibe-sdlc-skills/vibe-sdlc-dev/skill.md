...
依平台分派（github Projects vs gitlab Issue Boards，見 `../vibe-sdlc/references/git-platform-cli.md` §3）：

```bash
detect_remote
if [ "$PLATFORM" = "github" ]; then
  # GitHub Projects (v2)：取得 Issue 在 Project 中的 Item ID
  ...
  # GitLab Issue Boards：CLI 無等價直接更新指令，建議改以 label 標註狀態
  glab issue update <ISSUE_NUMBER> -R "$REPO_SLUG" --label "in-progress"
fi
```

> **提示**：若上述指令操作複雜（特別是 GitHub Projects v2 的 ID 取得流程），可改用 label 方式標註進度：
> - github: `gh issue edit <N> --add-label "in-progress"`
> - gitlab: `glab issue update <N> --label "in-progress"`
>
> 或直接在 Issue Comment 中說明狀態變更。選擇最簡單可行的方式即可。
...
2. **建立合併請求**：先呼叫 `detect_remote`，依平台分派（github: `gh pr create` / gitlab: `glab mr create`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.2），格式如下：
   - **標題**：`feat(<scope>): <簡述> (<任務編號>)`，例如 `feat(backend): Schema 與 DB 初始化 + 後端骨架 (T-101, T-102)`
   - **Body**：必須遵循 P4「PR/MR 格式規範」（變更摘要 / 關聯 Issue `Closes #N` 或 `Closes !N` / 變更清單 / 測試結果）
   - **Reviewer**：一律指定 H-Director 為 reviewer
     - github: `gh pr create ... --reviewer <H-Director-username>`
     - gitlab: `glab mr create ... --reviewer <H-Director-username>`
3. **回報合併請求連結**：在 Vibe Check 報告末尾附上 URL
...
合併指令會在當前分支被刪時自動把 HEAD 切到 base 分支（通常是 `main`）。為避免「不知不覺停留在 `main` 或被刪除的 feature branch」，**merge 指令執行後必須**依序執行：
...
```bash
# 標準收尾流程（依平台分派 merge，見 ../vibe-sdlc/references/git-platform-cli.md §2.2）
detect_remote
if [ "$PLATFORM" = "github" ]; then
  gh pr merge <N> --squash --auto --delete-branch
else
  glab mr merge <N> --squash-before-merge --remove-source-branch --auto-merge
fi
git fetch origin
...
| **2 — 建 Issue 再修正** | 先依平台分派建立 Issue（github: `gh issue create` / gitlab: `glab issue create`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.1），含重現步驟、截圖引用、嚴重程度標籤，然後立即修正，修正完成後詢問是否重新部署。 |
...
2. 先呼叫 `detect_remote`，再依平台讀取 Issue 內容（github: `gh issue view N -R "$REPO_SLUG"` / gitlab: `glab issue view N -R "$REPO_SLUG"`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.1）
3. 向開發者確認你對任務的理解，列出：
   - 任務範圍
   - 驗收標準
   - 需參考的規格文件
