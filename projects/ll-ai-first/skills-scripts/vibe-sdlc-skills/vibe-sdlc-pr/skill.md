...
| 步驟 | 執行者 | 操作 | 產出 |
|------|--------|------|------|
| 1 | **AI 助手** | 依平台分派監控 CI 結果（github: `gh pr checks` / gitlab: `glab ci status` 或 `glab mr view --output json` 取 `pipeline`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.2 / §2.5） | CI 狀態報告 |
...
若需對已建立 PR 的分支追加 commit（例如修正 CI 失敗、回應 Code Review 意見），**必須**在推送前執行（先呼叫 `detect_remote`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.2）：

```bash
# 檢查合併請求狀態
detect_remote
if [ "$PLATFORM" = "github" ]; then
  gh pr view <PR-NUMBER> -R "$REPO_SLUG" --json state -q '.state'
else
  glab mr view <PR-NUMBER> -R "$REPO_SLUG" --output json | jq -r '.state'
fi
```

| 狀態 | github | gitlab | 處理方式 |
|------|--------|--------|---------|
| 開啟中 | `OPEN` | `opened` | 正常推送至該分支，commit 會自動加入合併請求 |
| 已合併 | `MERGED` | `merged` | **禁止推送至該分支**。必須從最新 `main` 建立新分支、建立新合併請求 |
| 已關閉 | `CLOSED` | `closed` | 確認是否需要重新開啟，或建立新分支與新合併請求 |
...
1. 先確認前置條件：
   - 是否有已建立的 open 合併請求？（由 Phase 3 自動建立）
   - 若無 open 合併請求，提示使用者先完成 Phase 3（`/vibe-sdlc-dev`）
2. 先呼叫 `detect_remote`，再依平台列出所有 open 合併請求（github: `gh pr list` / gitlab: `glab mr list --opened`，見 `../vibe-sdlc/references/git-platform-cli.md` §2.2）
3. 監控 CI 結果（依平台分派，見 §2.2 / §2.5）：
   - github: `gh pr checks <N> -R "$REPO_SLUG"`
   - gitlab: `glab ci status -R "$REPO_SLUG"` 或 `glab mr view <N> --output json` 取 `pipeline` 欄位
   - CI 通過：通知開發者可進行 Code Review
   - CI 失敗：讀取失敗報告，分析原因，修正程式碼
   - **推送前必須確認合併請求狀態**（github: `gh pr view <N> --json state -q '.state'` / gitlab: `glab mr view <N> --output json | jq -r '.state'`）
     - 開啟中（`OPEN` / `opened`）→ 正常推送
     - 已合併（`MERGED` / `merged`）→ 禁止推送，從最新 main 建新分支與新合併請求
     - 已關閉（`CLOSED` / `closed`）→ 確認是否需重新開啟或建新合併請求
4. 開發者 Merge 後：
