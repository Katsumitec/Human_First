...
/vibe-sdlc-issues     ← 將計畫轉為 Issues（依 remote 自動分派 GitHub / GitLab）
...
你：/vibe-sdlc-issues
    規格定稿了，請建立 Issues。  # AI 依 remote 自動使用 gh 或 glab
...
- 平台 CLI（依專案 remote 擇一安裝，技能會自動偵測並分派）：
  - **GitHub 專案**：[GitHub CLI (`gh`)](https://cli.github.com/) — `brew install gh && gh auth login`
  - **GitLab 專案**：[GitLab CLI (`glab`)](https://gitlab.com/gitlab-org/cli) — `brew install glab && glab auth login --hostname <host>`
- 專案使用 GitHub 或 GitLab 作為版本管理平台（自架 GitLab 也支援，host 含 `gitlab` 字樣即自動辨識）

> 詳細命令對照與 `detect_remote` 偵測機制見 [`vibe-sdlc/references/git-platform-cli.md`](./vibe-sdlc/references/git-platform-cli.md)。
...
| `/vibe-sdlc-issues` | Phase 2：任務掛載 | 依 Dev Plan 建立 Issues（GitHub / GitLab），建立看板（Projects / Issue Boards） |
...
專案啟動
  └→ /vibe-sdlc-spec        撰寫規格 → 交叉比對審查 → 定稿
      └→ /vibe-sdlc-issues      審核計畫 → 建立 Issues（GitHub / GitLab）
...
將 Dev Plan 轉換為 **可追蹤的 Issues**（依平台分派為 GitHub Issues 或 GitLab Issues），並建立看板（GitHub Projects / GitLab Issue Boards）。

- **AI 職責**：`detect_remote` 偵測平台 → 確認對應 CLI 已登入 → 確認 P1 審查報告通過 → 建立 Labels 與 Milestones → 依里程碑建立開發 Issues → 掃描 `👁️ 手動` 驗證項建立驗證 Issues → 建立看板
...
- **Issue 生命週期**：開發任務由 PR/MR `Closes #N` 自動關閉；手動驗證由審查者附驗證結果後關閉；驗收門由 H-Director 關閉
- **看板差異**：
  - github: 建立後必須使用 `gh project link` 連結至 Repo
  - gitlab: Issue Boards 是 project-scope，無需「連結 repo」步驟；以 label 切分 swim lane
- **完成條件**：所有開發任務與驗證 Issues 皆已建立、Labels / Milestones 就緒、看板已關聯至 Repo
...
- **追加 Commit 安全檢查**：向已有合併請求的分支推送新 commit 前，必須依平台確認狀態（github: `gh pr view <N> --json state` / gitlab: `glab mr view <N> --output json`）。若已 merged 則禁止推送，須從最新 main 建新分支與新合併請求
...
- **PR 狀態安全檢查**：推送修正 commit 前必須依平台確認狀態（github: `gh pr view <N>` / gitlab: `glab mr view <N>`）為 OPEN/opened；若已合併則從最新 main 建新分支與新合併請求
...
AI 會自動偵測 remote 平台（GitHub / GitLab），收集 Issues、PR/MR、CI 狀態等數據，產出**進度儀表板**，包含：
- 各里程碑完成進度（進度條 + 百分比）
- 待審 PR 與 CI 狀態
...
### 場景三：Phase 2 — 建立 Issues（GitHub / GitLab 通用）

**審核 Dev Plan 並建立 Issues（審查報告通過後）：**

```
> /vibe-sdlc-issues
> 規格已經定稿了，審查報告也確認無誤，請幫我建立 Issues。
```

> AI 會先呼叫 `detect_remote` 判斷專案 remote 平台，自動使用 `gh` 或 `glab` 建立 Issues、Labels 與 Milestones。
...
AI 會逐一確認每筆 Issue 的標題、Labels、Milestone，依平台批量執行 `gh issue create` 或 `glab issue create`，接著直接領取 P0 Issue 進入 Phase 3 開發循環。若回報內容涉及 PRD / API Spec 變更，AI 會在處理完成後**即時回溯更新**對應規格文件並升版（核心原則第 4、5 條）。
...
- [GitHub CLI (`gh`)](https://cli.github.com/)：Phase 2~4 操作 GitHub 所需的 CLI（若使用 GitHub）
- [GitLab CLI (`glab`)](https://gitlab.com/gitlab-org/cli)：Phase 2~4 操作 GitLab 所需的 CLI（若使用 GitLab）
- [`vibe-sdlc/references/git-platform-cli.md`](./vibe-sdlc/references/git-platform-cli.md)：gh / glab 完整對照表與 `detect_remote` 偵測邏輯
