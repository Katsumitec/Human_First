...
4. **建立 Release**（先呼叫 `detect_remote`，依平台分派；見 `../vibe-sdlc/references/git-platform-cli.md` §2.3）：
   ```bash
   detect_remote
   if [ "$PLATFORM" = "github" ]; then
     gh release create {version} -R "$REPO_SLUG" \
       --title "{version} - {project_name}" \
       [--prerelease] \
       --notes "..."
   else
     glab release create {version} -R "$REPO_SLUG" \
       --name "{version} - {project_name}" \
       --notes "..."
     # GitLab 無 --prerelease 旗標，預發佈版本以 tag 命名（如 v1.0.0-rc.1）表達
   fi
   ```
...
| 風格 | 內容 |
|------|------|
| **自動生成** | github: `gh release create --generate-notes` 自動從 PR 標題生成。gitlab: `glab` 無對應旗標，改以 `git log` 整理過往合併紀錄產出 notes |
...
- [ ] Release 已發佈至 git 平台（GitHub Release / GitLab Release，若開發者選擇發佈）
